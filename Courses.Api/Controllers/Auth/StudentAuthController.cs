...
namespace Courses.Api.Controllers.Auth
{
    ...
    public class StudentAuthController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResponse<LoginResponseDto>>> Login([FromBody] LoginRequestDto request)
        {
            try
            {
                var query = new LoginQuery(request, UserType.Student);
                var result = await _mediator.Send(query);
                return this.Success(result, "Login successful");
            }
            catch (UnauthorizedAccessException ex)
            {
                _logger.LogWarning("Student login failed: {Message}", ex.Message);
                return this.Unauthorized<LoginResponseDto>(ex.Message);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Unexpected error during student login");
        ...
        public async Task<ActionResult<ApiResponse<RegisterResponseDto>>> Register([FromBody] StudentRegisterRequestDto request)
        {
            try
            {
                var command = new StudentRegisterCommand(request);
                var result = await _mediator.Send(command);
                return this.Success(result, "Registration successful");
            }
            catch (InvalidOperationException ex)
            {
                _logger.LogWarning("Student registration failed: {Message}", ex.Message);
                return this.BadRequest<RegisterResponseDto>(ex.Message);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Unexpected error during student registration");
        ...
        public async Task<ActionResult<ApiResponse<VerifyEmailResponseDto>>> VerifyEmail([FromBody] VerifyCodeRequestDto request)
        {
            try
            {
                var command = new VerifyEmailCommand(request, UserType.Student);
                var result = await _mediator.Send(command);
                return this.Success(result, "Email verified successfully");
            }
            catch (InvalidOperationException ex)
            {
                _logger.LogWarning("Student email verification failed: {Message}", ex.Message);
                return this.BadRequest<VerifyEmailResponseDto>(ex.Message);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Unexpected error during student email verification");
        ...
        public async Task<ActionResult<ApiResponse<object>>> ResendVerification([FromBody] SendVerificationCodeRequestDto request)
        {
            try
            {
                var command = new ResendVerificationCodeCommand(request, UserType.Student);
                await _mediator.Send(command);
                return this.Success<object>(null!, "Verification code sent to your email");
            }
            catch (InvalidOperationException ex)
            {
                _logger.LogWarning("Student resend verification failed: {Message}", ex.Message);
                return this.BadRequest<object>(ex.Message);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Unexpected error during student resend verification");
        ...
        public async Task<ActionResult<ApiResponse<UserInfoDto>>> GetCurrentStudent()
        {
            try
            {
                ...
                var query = new GetCurrentUserQuery(userId, UserType.Student);
                var result = await _mediator.Send(query);
                return this.Success(result, "User information retrieved successfully");
            }
            catch (InvalidOperationException ex)
            {
                _logger.LogWarning("GetCurrentStudent failed: {Message}", ex.Message);
            ...
            {
                _logger.LogError(ex, "Unexpected error during GetCurrentStudent");
