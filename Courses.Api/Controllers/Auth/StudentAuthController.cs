namespace Courses.Api.Controllers.Auth
{
    ...
    public class StudentAuthController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResponse<LoginResponseDto>>> Login([FromBody] LoginRequestDto request)
        {
            ...
            {
                _logger.LogWarning("Login failed: Non-student user attempted to login with email {Email}", request.Email);
                return this.Unauthorized<LoginResponseDto>("This login is for students only");
            }
            ...
            if (!user.IsActive)
            {
                _logger.LogWarning("Login failed: Deactivated account for email {Email}", request.Email);
        ...
        [HttpPost("register")]
        public async Task<ActionResult<ApiResponse<RegisterResponseDto>>> Register([FromBody] StudentRegisterRequestDto request)
        {
            ...
            var user = new ApplicationUser
            {
                ...
                UserType = UserType.Student, // Force student type
            ...
            {
                _logger.LogError(ex, "Error sending welcome email or verification code for user {UserId}", user.Id);
        ...
        public async Task<ActionResult<ApiResponse<VerifyEmailResponseDto>>> VerifyEmail([FromBody] VerifyCodeRequestDto request)
        {
            ...
            if (user.UserType != UserType.Student)
            {
                _logger.LogWarning("Email verification failed: Non-student user attempted verification for email {Email}", request.Email);
                return this.BadRequest<VerifyEmailResponseDto>("This verification is for students only");
            }

            var isValid = await _twoFactorService.ValidateVerificationCodeAsync(user, request.Code);
            if (!isValid)
            {
                _logger.LogWarning("Email verification failed: Invalid code for email {Email}", request.Email);
        ...
        public async Task<ActionResult<ApiResponse<object>>> ResendVerification([FromBody] SendVerificationCodeRequestDto request)
        {
            ...
            if (user.UserType != UserType.Student)
            {
                _logger.LogWarning("Resend verification failed: Non-student user attempted for email {Email}", request.Email);
                return this.BadRequest<object>("This verification is for students only");
            }

            if (user.EmailConfirmed)
            {
                _logger.LogWarning("Resend verification failed: Email already verified for {Email}", request.Email);
            ...
            if (!emailSent)
            {
                _logger.LogError("Resend verification failed: Could not send code for email {Email}", request.Email);
        ...
        [HttpGet("me")]
        public async Task<ActionResult<ApiResponse<UserInfoDto>>> GetCurrentStudent()
        {
            var userId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            if (string.IsNullOrEmpty(userId))
            {
                _logger.LogWarning("GetCurrentStudent failed: No user ID in token");
            ...
            if (user == null)
            {
                _logger.LogWarning("GetCurrentStudent failed: User not found for ID {UserId}", userId);
            ...
            if (user.UserType != UserType.Student)
            {
                _logger.LogWarning("GetCurrentStudent failed: Non-student user attempted access {UserId}", userId);
            ...
            _logger.LogInformation("GetCurrentStudent successful for user: {UserId}", user.Id);
